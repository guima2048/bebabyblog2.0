...
export default function PostContent({ slug }: { slug: string }) {
  ...
  const [newCommentTrigger, setNewCommentTrigger] = useState(0);
  ...
  const handleCommentAdded = () => {
    setNewCommentTrigger(prev => prev + 1);
  };
  ...
  return (
    <article className="min-h-screen bg-[#e9d8fd] py-16 px-4">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg border border-[#c4b5fd] p-8">
        ...
        <CommentsList slug={post.slug} newCommentTrigger={newCommentTrigger} />
        <CommentForm slug={post.slug} onCommentAdded={handleCommentAdded} />
